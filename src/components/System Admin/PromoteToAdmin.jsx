...
const PromoteAdmin = () => {
  ...
  const handlePromote = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/v1/roles/${userId}/promote/admin`, {
        method: "POST",
        credentials:"include",
        headers: {
      ...
      if (response.ok) {
        setMessage({ type: "success", text: data.message || "User promoted to Admin successfully." });
      } else {
